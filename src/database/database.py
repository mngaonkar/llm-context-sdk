...
class Database():
    def __init__(self, db_name = "milvus_demo.db", collection_name = "milvus_demo"):
        ...
        self.embeddings = GPT4AllEmbeddings(model_name=EMBEDDING_MODEL) # type: ignore
    ...
    def query_document(self, query):
        ...
        docs = self.vector_db.similarity_search_with_score(query, k=NO_DOCS_PER_QUERY)
        logger.info(f"Found {len(docs)} documents.")
        return docs
    
    def list_databases(self):
        """Get all the database."""
        all_files = os.listdir(DOCS_LOCATION)
